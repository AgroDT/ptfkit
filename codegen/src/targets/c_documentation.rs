...
use super::{
    GeneratedFile,
    documentation::{self as markdown, HEADER},
    group_by_source,
...
fn render_index(
    ...
    writer.blank_line();
    writer.line("ptfkit's C API is organized around installed headers.");
    writer.blank_line();
    writer.line("## Headers");
    writer.blank_line();
    writer.line(
...
fn render_umbrella(
    ...
    writer.write(HEADER);
    writer.line("# `<ptfkit/ptfkit.h>`");
    writer.blank_line();
    markdown::code_block(writer, "c", |writer| {
        writer.line("#include <ptfkit/ptfkit.h>");
    });
    writer.line("This umbrella header aggregates every public ptfkit source header. Include an individual header when only one source is needed.");
    writer.blank_line();
    writer.line("## Included headers");
    writer.blank_line();
    for (slug, functions) in sources {
        writer.line(format_args!(
...
fn render_header(writer: &mut Writer, slug: &str, functions: &[&CompiledFunction]) -> Result<()> {
    ...
    writer.write(HEADER);
    writer.line(format_args!("# `<ptfkit/{slug}.h>`"));
    writer.blank_line();
    markdown::code_block(writer, "c", |writer| {
...
fn render_function_documentation(writer: &mut Writer, function: &CompiledFunction) -> Result<()> {
    ...
    writer.blank_line();
    writer.line("| Name | Direction | Description |");
    writer.line("| --- | --- | --- |");
    ...
    match document.returns {
        ...
        docs::Returns::Record { .. } => {
            let name = spec
                .result_class()
                .expect("record output has a result class");
            writer.line(format_args!("A `{}` value.", c_result_name(name)));
            writer.blank_line();
        }
    }
...
fn render_functions_index(writer: &mut Writer, functions: &[(&str, &CompiledFunction)]) {
    ...
    writer.blank_line();
    writer.line("| Function | Summary | Header |");
    writer.line("| --- | --- | --- |");
