...
use crate::{
    ...
    model::{CompiledFunction, CompiledInput, EnumDefinition, Output},
    semantic::{
    ...
    group_by_source(functions)
        ...
        .map(|(slug, functions)| {
            ...
            let enum_definitions = functions
                ...
                .filter(|definition| enum_names.insert(definition.name.as_str()))
                .map(enum_tokens)
                .collect::<Vec<_>>();
...
fn module_tokens(
    ...
    let input_types = specification
        ...
        .map(|input| {
            input
                .enum_type()
                .map(|definition| format_ident!("{}", definition.name))
                .map_or_else(|| quote!(f64), |name| quote!(#name))
        })
        .collect::<Vec<_>>();
...
fn lookup_conversion_tokens(functions: &[&CompiledFunction]) -> Vec<TokenStream> {
    ...
    functions
        ...
        .map(|lookup| {
            let enum_name = format_ident!("{}", lookup.enum_name);
