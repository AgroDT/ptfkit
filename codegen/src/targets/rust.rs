...
pub(crate) fn render_with_definitions(
    ...
    let shared_modules = shared.keys().copied().collect::<Vec<_>>();
    let source_modules = sources.keys().copied().collect::<Vec<_>>();
    let mut generated = vec![GeneratedFile::new(
        "lib.rs".into(),
        crate_root(&shared_modules, &source_modules),
    )];
    for (document, definitions) in &shared {
        let module_docs = documents
            .iter()
        ...
        generated.push(GeneratedFile::new(
            PathBuf::from(format!("{document}.rs")),
            render_tokens(module_docs, quote!(#(#definitions)*)),
        ));
    }
    generated.extend(sources
        .into_iter()
        .map(|(slug, functions)| {
            let first = functions
                .first()
...
fn crate_root(shared_modules: &[&str], source_modules: &[&str]) -> String {
    let mut output = format!(
        "{HEADER}\n//! Pedotransfer functions for estimating soil hydraulic properties.\n\n\
         pub const VERSION: &str = env!(\"CARGO_PKG_VERSION\");\n"
    );
    for (comment, modules) in [
        ("Shared domain modules.", shared_modules),
        ("Source publication modules.", source_modules),
    ] {
        if !modules.is_empty() {
...
fn enum_tokens(definition: &EnumDefinition) -> TokenStream {
    ...
    let docs = doc_tokens([definition.description.clone()]);
    ...
    quote!(#docs #[derive(Clone, Copy, Debug, Eq, PartialEq)] pub enum #name { #(#members),* })
}
...
mod tests {
    ...
    #[test]
    fn crate_root_registers_every_source_once_without_shared_definitions() {
        let mut functions = crate::test_support::functions();
        functions.reverse();
        let files = render(&functions).unwrap();
        let root = files
            .iter()
            .find(|file| file.path == std::path::Path::new("lib.rs"))
            .unwrap();
        snapbox::assert_data_eq!(
            &root.contents,
            snapbox::file!["../fixtures/expected/rust/crate_root_sources.rs"]
        );
    }

    #[test]
    fn crate_root_registers_new_source_and_shared_modules_through_output_commit() {
        let root = crate::test_support::fixture_root("rust-crate-root");
        crate::test_support::copy_shared_definition_fixture(&root);
        std::fs::rename(
            root.join("specs/definitions/soil.yaml"),
            root.join("specs/definitions/water.yaml"),
        )
        .unwrap();
        for source in ["first_source", "second_source"] {
            let path = root.join(format!("specs/functions/{source}.yaml"));
            let contents = std::fs::read_to_string(&path).unwrap();
            std::fs::write(path, contents.replace("soil.yaml", "water.yaml")).unwrap();
        }
        let entries = crate::load_validated_specifications(&root).unwrap();
        let functions = crate::compile::functions(entries).unwrap();
        let files = render(&functions).unwrap();
        let target = root.join("targets/ptfkit-rs/src");
        std::fs::create_dir_all(&target).unwrap();
        let helper = "// Handwritten helper.\npub fn untouched( ) {}\n";
        std::fs::write(target.join("test_support.rs"), helper).unwrap();
        crate::output::commit(
            &root,
            &[crate::output::Output::new(&crate::output::RUST, files)],
        )
        .unwrap();
        snapbox::assert_data_eq!(
            std::fs::read_to_string(target.join("lib.rs")).unwrap(),
            snapbox::file!["../fixtures/expected/rust/crate_root_shared.rs"]
        );
        assert_eq!(
            std::fs::read_to_string(target.join("test_support.rs")).unwrap(),
            helper
        );
    ...
    #[test]
    fn crate_root_preserves_infrastructure_for_an_empty_corpus() {
        let files = render(&[]).unwrap();
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, PathBuf::from("lib.rs"));
        snapbox::assert_data_eq!(
            &files[0].contents,
            snapbox::file!["../fixtures/expected/rust/crate_root_empty.rs"]
        );
    }
