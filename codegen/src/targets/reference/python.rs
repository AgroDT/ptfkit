...
pub(crate) fn render(entries: &[Entry]) -> Vec<GeneratedFile> {
    ...
    files.push(markdown::markdown_file("usda_texture.md", |writer| {
        markdown::generated_frontmatter(writer, |writer| {
            writer.line("title: Python module ptfkit.usda_texture");
            writer.line("nav-title: ptfkit.usda_texture");
        });
        writer.line("::: ptfkit.usda_texture");
    }));
...
mod tests {
    ...
    fn renders_one_page_for_each_source_module() {
        ...
        assert_eq!(files.len(), entries.len() + 2);
        assert!(index.contains("[`ptfkit.usda_texture`](usda_texture.md)"));
        assert!(contents(&files, "usda_texture.md").contains("::: ptfkit.usda_texture"));
