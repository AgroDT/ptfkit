...
Versioned native source archives are attached to each [GitHub Release](https://github.com/AgroDT/ptfkit/releases). For a release `vX.Y.Z`, download `ptfkit-native-X.Y.Z.tar.gz` and its matching `.sha256` file, verify the checksum, extract the archive, and build it with CMake.
