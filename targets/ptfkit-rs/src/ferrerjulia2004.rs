// @generated by ptfkit-codegen; DO NOT EDIT.

#![doc = r"Ferrer Julià et al. (2004), saturated-conductivity PTFs for Spanish soils.

# Reference

Ferrer Julià, M., Estrela Monreal, T., Sánchez del Corral Jiménez, A., & García Meléndez,
E. (2004). Constructing a saturated hydraulic conductivity map of Spain using pedotransfer
functions and spatial prediction. Geoderma, 123, 257-277.
DOI: 10.1016/j.geoderma.2004.02.011 (https://doi.org/10.1016/j.geoderma.2004.02.011)

# Territory

Spanish mainland on the Iberian Peninsula

# Dataset

Trueba et al. (2000a) Spanish soil database: 2178 profiles and 7011 horizons; the general
regressions used 3172 horizons with sufficient data."]

#[doc = r"Evaluate the Campbell and Shiozawa saturated-conductivity PTF.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Campbell and Shiozawa (1994)

# Notes

Prediction target: Saturated hydraulic conductivity.
Reproduced in Section 4.2 for comparison with the new Spanish-soil PTFs."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_campbell_shiozawa(sand: f64, clay: f64) -> f64 {
    54.0f64 * (-0.07f64 * sand - 0.167f64 * clay).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_campbell_shiozawa_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_campbell_shiozawa(50f64, 25f64);
        assert_in_interval(result, 0.025071690110308728f64, 0.025071690110309172f64);
    }
}
#[doc = r"Evaluate the Saxton et al. saturated-conductivity PTF.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage; strictly positive for the base-10 logarithm. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Saxton et al. (1986)

# Notes

Prediction target: Saturated hydraulic conductivity.
Reproduced in Section 4.2 for comparison with the new Spanish-soil PTFs.

# Warnings

The denominator contains log10(clay) and the source does not state its valid domain or
singularity policy."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_saxton(sand: f64, clay: f64) -> f64 {
    10.0f64
        * (1.01f64 - 0.0755f64 * sand
            + (-3.895f64 + 0.03671f64 * sand - 0.1103f64 * clay + 0.00087546f64 * clay.powi(2))
                / (0.33f64 - 0.000751f64 * sand + 0.176f64 * clay.log10()))
        .exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_saxton_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_saxton(50f64, 25f64);
        assert_in_interval(result, 0.00022673430784327062f64, 0.0002267343078432741f64);
    }
}
#[doc = r"Evaluate the Dane and Puckett saturated-conductivity PTF.

# Arguments

  * clay: Clay content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Dane and Puckett (1994)

# Notes

Prediction target: Saturated hydraulic conductivity.
Reproduced in Section 4.2 for comparison with the new Spanish-soil PTFs."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_dane_puckett(clay: f64) -> f64 {
    303.84f64 * (-0.144f64 * clay).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_dane_puckett_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_dane_puckett(25f64);
        assert_in_interval(result, 8.302039828385258f64, 8.302039828385485f64);
    }
}
#[doc = r"Evaluate the Puckett et al. saturated-conductivity PTF.

# Arguments

  * clay: Clay content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Puckett et al. (1985)

# Notes

Prediction target: Saturated hydraulic conductivity.
Reproduced in Section 4.2 for comparison with the new Spanish-soil PTFs."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_puckett(clay: f64) -> f64 {
    156.96f64 * (-0.1975f64 * clay).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_puckett_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_puckett(25f64);
        assert_in_interval(result, 1.1257967371765512f64, 1.1257967371765796f64);
    }
}
#[doc = r"Evaluate the Cosby et al. saturated-conductivity PTF.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Cosby et al. (1984)

# Notes

Prediction target: Saturated hydraulic conductivity.
Reproduced in Section 4.2 for comparison with the new Spanish-soil PTFs."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_cosby(sand: f64, clay: f64) -> f64 {
    25.4f64 + 10.0f64.powf(-0.6f64 + 0.012f64 * sand - 0.0064f64 * clay)
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_cosby_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_cosby(50f64, 25f64);
        assert_in_interval(result, 26.09183097091871f64, 26.091830970919165f64);
    }
}
#[doc = r"Estimate saturated conductivity for Humic Acrisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Humic Acrisol.
Table 3; R^2 = 0.723."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_humic_acrisol_sand(sand: f64) -> f64 {
    1.5162f64 * (0.0452f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_humic_acrisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_humic_acrisol_sand(50f64);
        assert_in_interval(result, 14.529879794648036f64, 14.529879794648263f64);
    }
}
#[doc = r"Estimate saturated conductivity for Humic Acrisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Humic Acrisol.
Table 3; R^2 = 0.701."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_humic_acrisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    16.019f64 + 0.345f64 * sand - 0.548f64 * clay - 0.00221f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_humic_acrisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_humic_acrisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 19.563474999999972f64, 19.56347500000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcic Cambisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcic Cambisol.
Table 3; R^2 = 0.521."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcic_cambisol_sand(sand: f64) -> f64 {
    1.043f64 * (0.0452f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcic_cambisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_calcic_cambisol_sand(50f64);
        assert_in_interval(result, 9.995162000935132f64, 9.995162000935359f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcic Cambisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcic Cambisol.
Table 3; R^2 = 0.468."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcic_cambisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -7.878f64 + 0.51f64 * sand - 0.00183f64 * clay - 0.424f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcic_cambisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_calcic_cambisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 16.51624999999997f64, 16.516250000000028f64);
    }
}
#[doc = r"Estimate saturated conductivity for Dystric Cambisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Dystric Cambisol.
Table 3; R^2 = 0.750."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_dystric_cambisol_sand(sand: f64) -> f64 {
    1.3182f64 * (0.0464f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_dystric_cambisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_dystric_cambisol_sand(50f64);
        assert_in_interval(result, 13.413573870265756f64, 13.413573870265983f64);
    }
}
#[doc = r"Estimate saturated conductivity for Dystric Cambisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Dystric Cambisol.
Table 3; R^2 = 0.779."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_dystric_cambisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -5.125f64 + 0.631f64 * sand - 0.165f64 * clay - 0.604f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_dystric_cambisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_dystric_cambisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 20.78999999999997f64, 20.790000000000028f64);
    }
}
#[doc = r"Estimate saturated conductivity for Eutric Cambisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Eutric Cambisol.
Table 3; R^2 = 0.734."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_eutric_cambisol_sand(sand: f64) -> f64 {
    0.9356f64 * (0.0503f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_eutric_cambisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_eutric_cambisol_sand(50f64);
        assert_in_interval(result, 11.570199173737253f64, 11.57019917373748f64);
    }
}
#[doc = r"Estimate saturated conductivity for Eutric Cambisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Eutric Cambisol.
Table 3; R^2 = 0.686."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_eutric_cambisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -0.399f64 + 0.541f64 * sand - 0.243f64 * clay - 0.421f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_eutric_cambisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_eutric_cambisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 19.52349999999997f64, 19.523500000000027f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Cambisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Cambisol.
Table 3; R^2 = 0.807."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_cambisol_sand(sand: f64) -> f64 {
    0.2336f64 * (0.0667f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_cambisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_gleyic_cambisol_sand(50f64);
        assert_in_interval(result, 6.559110321079869f64, 6.559110321079983f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Cambisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Cambisol.
Table 3; R^2 = 0.824."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_cambisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -12.986f64 + 0.638f64 * sand - 0.244f64 * clay - 0.52f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_cambisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_gleyic_cambisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 11.513999999999985f64, 11.514000000000014f64);
    }
}
#[doc = r"Estimate saturated conductivity for Humic Cambisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Humic Cambisol.
Table 3; R^2 = 0.592."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_humic_cambisol_sand(sand: f64) -> f64 {
    2.1622f64 * (0.0371f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_humic_cambisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_humic_cambisol_sand(50f64);
        assert_in_interval(result, 13.820129957492394f64, 13.820129957492622f64);
    }
}
#[doc = r"Estimate saturated conductivity for Humic Cambisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Humic Cambisol.
Table 3; R^2 = 0.632."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_humic_cambisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    6.4429f64 + 0.475f64 * sand - 0.331f64 * clay - 0.406f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_humic_cambisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_humic_cambisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 20.90289999999997f64, 20.902900000000027f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcaric Fluvisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcaric Fluvisol.
Table 3; R^2 = 0.772."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcaric_fluvisol_sand(sand: f64) -> f64 {
    0.814f64 * (0.052f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcaric_fluvisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_calcaric_fluvisol_sand(50f64);
        assert_in_interval(result, 10.959482760491262f64, 10.95948276049149f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcaric Fluvisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcaric Fluvisol.
Table 3; R^2 = 0.792."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcaric_fluvisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -15.779f64 + 0.657f64 * sand - 0.134f64 * clay - 0.248f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcaric_fluvisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_calcaric_fluvisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 13.100999999999987f64, 13.101000000000015f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcic Luvisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcic Luvisol.
Table 3; R^2 = 0.412.

# Warnings

The paper reports weak fit and suggests missing clay-mineralogy information."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcic_luvisol_sand(sand: f64) -> f64 {
    0.671f64 * (0.0467f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcic_luvisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_calcic_luvisol_sand(50f64);
        assert_in_interval(result, 6.931067612606899f64, 6.931067612607013f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcic Luvisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcic Luvisol.
Table 3; R^2 = 0.473.

# Warnings

The paper reports weak fit and suggests missing clay-mineralogy information."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcic_luvisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    5.379f64 + 0.138f64 * sand - 0.131f64 * clay - 0.02832f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcic_luvisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_calcic_luvisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 8.933199999999985f64, 8.933200000000014f64);
    }
}
#[doc = r"Estimate saturated conductivity for Chromic Luvisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Chromic Luvisol.
Table 3; R^2 = 0.534."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_chromic_luvisol_sand(sand: f64) -> f64 {
    0.898f64 * (0.0494f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_chromic_luvisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_chromic_luvisol_sand(50f64);
        assert_in_interval(result, 10.616557272778318f64, 10.616557272778545f64);
    }
}
#[doc = r"Estimate saturated conductivity for Chromic Luvisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Chromic Luvisol.
Table 3; R^2 = 0.557."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_chromic_luvisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -10.171f64 + 0.576f64 * sand - 0.0075f64 * clay - 0.518f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_chromic_luvisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_chromic_luvisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 17.14649999999997f64, 17.146500000000028f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Luvisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Luvisol.
Table 3; R^2 = 0.876."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_luvisol_sand(sand: f64) -> f64 {
    0.5395f64 * (0.0514f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_luvisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_gleyic_luvisol_sand(50f64);
        assert_in_interval(result, 7.049012285884137f64, 7.049012285884251f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Luvisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Luvisol.
Table 3; R^2 = 0.917."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_luvisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    10.28f64 + 0.449f64 * sand - 0.507f64 * clay - 0.202f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_luvisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_gleyic_luvisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 19.549999999999972f64, 19.55000000000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Orthic Luvisol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Orthic Luvisol.
Table 3; R^2 = 0.676."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_orthic_luvisol_sand(sand: f64) -> f64 {
    1.3958f64 * (0.0431f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_orthic_luvisol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_orthic_luvisol_sand(50f64);
        assert_in_interval(result, 12.042809111804472f64, 12.042809111804699f64);
    }
}
#[doc = r"Estimate saturated conductivity for Orthic Luvisol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Orthic Luvisol.
Table 3; R^2 = 0.729."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_orthic_luvisol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    4.829f64 + 0.499f64 * sand - 0.332f64 * clay - 0.0299f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_orthic_luvisol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_orthic_luvisol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 21.404249999999973f64, 21.40425000000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Ranker from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Ranker.
Table 3; R^2 = 0.726."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_ranker_sand(sand: f64) -> f64 {
    0.9847f64 * (0.0507f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_ranker_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_ranker_sand(50f64);
        assert_in_interval(result, 12.423399456061821f64, 12.423399456062048f64);
    }
}
#[doc = r"Estimate saturated conductivity for Ranker from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Ranker.
Table 3; R^2 = 0.717."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_ranker_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -8.621f64 + 0.682f64 * sand - 0.0771f64 * clay - 0.44f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_ranker_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_ranker_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 22.45149999999997f64, 22.451500000000028f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcaric Regosol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcaric Regosol.
Table 3; R^2 = 0.655."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcaric_regosol_sand(sand: f64) -> f64 {
    0.8931f64 * (0.0524f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcaric_regosol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_calcaric_regosol_sand(50f64);
        assert_in_interval(result, 12.26737473383298f64, 12.267374733833208f64);
    }
}
#[doc = r"Estimate saturated conductivity for Calcaric Regosol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Calcaric Regosol.
Table 3; R^2 = 0.705."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_calcaric_regosol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    1.522f64 + 0.537f64 * sand - 0.284f64 * clay - 0.296f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_calcaric_regosol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_calcaric_regosol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 20.53199999999997f64, 20.53200000000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Dystric Regosol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Dystric Regosol.
Table 3; R^2 = 0.834."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_dystric_regosol_sand(sand: f64) -> f64 {
    1.096f64 * (0.048f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_dystric_regosol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_dystric_regosol_sand(50f64);
        assert_in_interval(result, 12.08140131318308f64, 12.081401313183308f64);
    }
}
#[doc = r"Estimate saturated conductivity for Dystric Regosol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Dystric Regosol.
Table 3; R^2 = 0.862."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_dystric_regosol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -0.784f64 + 0.596f64 * sand - 0.431f64 * clay - 0.443f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_dystric_regosol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_dystric_regosol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 17.133499999999973f64, 17.13350000000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Eutric Regosol from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Eutric Regosol.
Table 3; R^2 = 0.824."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_eutric_regosol_sand(sand: f64) -> f64 {
    1.0005f64 * (0.0523f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_eutric_regosol_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_eutric_regosol_sand(50f64);
        assert_in_interval(result, 13.674049986082617f64, 13.674049986082844f64);
    }
}
#[doc = r"Estimate saturated conductivity for Eutric Regosol from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Eutric Regosol.
Table 3; R^2 = 0.702."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_eutric_regosol_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -5.5f64 + 0.647f64 * sand - 0.00325f64 * clay - 1.082f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_eutric_regosol_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_eutric_regosol_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 24.06374999999997f64, 24.063750000000027f64);
    }
}
#[doc = r"Estimate saturated conductivity for Rendzina from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Rendzina.
Table 3; horizon-level R^2 = 0.221; the paper separately reports profile-average R^2 = 0.727
without coefficients.

# Warnings

The horizon-level regression has weak fit and is affected by converted qualitative conductivity
values."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_rendzina_sand(sand: f64) -> f64 {
    2.4645f64 * (0.0325f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_rendzina_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_rendzina_sand(50f64);
        assert_in_interval(result, 12.515763717130197f64, 12.515763717130424f64);
    }
}
#[doc = r"Estimate saturated conductivity for Rendzina from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Rendzina.
Table 3; R^2 = 0.340.

# Warnings

The regression has weak fit and is affected by converted qualitative conductivity values."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_rendzina_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    23.288f64 + 0.175f64 * sand - 0.602f64 * clay - 0.279f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_rendzina_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_rendzina_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 16.290499999999973f64, 16.29050000000003f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Solonchak from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Solonchak.
Table 3 prints R^2 = 0.399; the prose later states 0.435.

# Warnings

The source gives inconsistent R^2 values for this regression."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_solonchak_sand(sand: f64) -> f64 {
    0.7602f64 * (0.0372f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_solonchak_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_gleyic_solonchak_sand(50f64);
        assert_in_interval(result, 4.883324693640371f64, 4.883324693640485f64);
    }
}
#[doc = r"Estimate saturated conductivity for Gleyic Solonchak from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for FAO 1974 Gleyic Solonchak.
Table 3; R^2 = 0.682.
Human review corrected the anomalous printed intercept from -90917 to -0.90917.

# Warnings

The implementation intentionally differs from the original published Table 3
which prints the intercept as -90917."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_gleyic_solonchak_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -0.90917f64 + 0.328f64 * sand - 0.173f64 * clay - 0.252f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_gleyic_solonchak_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result =
            calc_ptf_ferrerjulia2004_gleyic_solonchak_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 10.535829999999986f64, 10.535830000000015f64);
    }
}
#[doc = r"Estimate saturated conductivity for Spanish soils from sand content.

# Arguments

  * sand: Sand content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Exponential one-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for the combined Spanish-soil dataset.
Section 5.1; 3172 horizons; R^2 = 0.696.

# Warnings

The paper recommends use when data are statistically similar to its Spanish-soil calibration
data."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_general_sand(sand: f64) -> f64 {
    0.920f64 * (0.0491f64 * sand).exp()
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_general_sand_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_general_sand(50f64);
        assert_in_interval(result, 10.714718864968997f64, 10.714718864969225f64);
    }
}
#[doc = r"Estimate saturated conductivity for Spanish soils from texture and organic matter.

# Arguments

  * sand: Sand content by percentage. (%)
  * clay: Clay content by percentage. (%)
  * organic_matter: Organic matter content by percentage. (%)

# Returns

  * k_sat: Estimated saturated hydraulic conductivity. (mm/h)

# Models

  * k(h): Linear three-variable regression

# Notes

Prediction target: Saturated hydraulic conductivity for the combined Spanish-soil dataset.
Section 5.1; 3172 horizons; R^2 = 0.715.

# Warnings

The paper recommends use when data are statistically similar to its Spanish-soil calibration
data."]
#[cfg_attr(feature = "inline", inline)]
#[must_use]
pub fn calc_ptf_ferrerjulia2004_general_texture_organic_matter(
    sand: f64,
    clay: f64,
    organic_matter: f64,
) -> f64 {
    -4.994f64 + 0.56728f64 * sand - 0.131f64 * clay - 0.0127f64 * organic_matter
}
#[cfg(test)]
mod calc_ptf_ferrerjulia2004_general_texture_organic_matter_tests {
    use super::*;
    fn assert_in_interval(actual: f64, lower: f64, upper: f64) {
        assert!(
            actual >= lower && actual <= upper,
            "actual {actual} is outside [{lower}, {upper}]"
        );
    }
    #[test]
    fn reviewer_reference_vector() {
        let result = calc_ptf_ferrerjulia2004_general_texture_organic_matter(50f64, 25f64, 2.5f64);
        assert_in_interval(result, 20.06324999999997f64, 20.06325000000003f64);
    }
}
